...
| 食事 | プリセット選択・手入力・当日一覧・削除 | FR-010〜013 |
...
| MealLog | 食事記録 | 記録日、名称、kcal、P/F/C、プリセット参照（任意） |
...
| 連携先 | 方向 | 目的 |
|--------|------|------|
| iPhone ショートカット | Inbound HTTP | 歩数・体重同期（Phase 1） |
| Open Food Facts | Outbound HTTPS | バーコード食品（Phase 2） |
